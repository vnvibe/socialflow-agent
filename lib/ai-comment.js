...
- Gợi ý/Nhắc tới ${mentionTarget} một cách cực kỳ tự nhiên, khéo léo theo góc: ${angle}
- Đóng vai là người dùng đã sử dụng và chia sẻ trải nghiệm thực tế về nó để giúp tác giả bài viết
${existingComments.length > 0 ? `- Nếu đã có người suggest ${mentionTarget} → KHÔNG nhắc lại, hãy bổ sung thông tin khác (giá, trải nghiệm, so sánh)` : ''}
