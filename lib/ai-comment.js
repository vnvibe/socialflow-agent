...
async function callHermesComment(payload) {
...
async function callHermesQualityGate(payload) {
...
async function generateComment(context = {}) {
  const { postText, groupName, topic, style, userId, language } = context
        ...
        if (!comment || comment.length < 10 || /^\.+$/.test(comment)) {
          ...
          try {
            ...
            if (gate && gate.pass === false) {
              ...
              // Return empty rather than post bad comment
