...
// aff (tuỳ chọn, 04/09 — SaaS đa tenant): lexicon PER-USER từ lib/user-affinity
// {high: Set, all: []}. Không truyền → dùng bộ legacy toàn cục (tương thích
// ngược cho health-check và code cũ; bộ legacy là ngách VPS của farm gốc).
function isHighAffinity(kw, aff = null) {
  if (kw == null) return false
  const s = stripDiacritics(kw)
  return aff ? aff.high.has(s) : HIGH_AFFINITY.has(s)
}

function matchesAffinity(normText, aff = null) {
  // Ưu tiên trả về từ khoá GẦN CAMP nếu bài khớp cả 2 nhóm (để tier/adjacency
  // phản ánh đúng mức liên quan camp).
  const danhSach = aff ? aff.all : TECH_AFFINITY
  const high = aff ? aff.high : HIGH_AFFINITY
  let low = null
  for (const kw of danhSach) {
    if (matchesWholeWord(normText, kw)) {
      if (high.has(kw)) return kw
      if (!low) low = kw
    }
  }
  return low
...
// Bài đời sống/lifestyle, làm đẹp, giải trí — dù có chứa từ khoá tech (như 'ChatGPT', 'AI')
// thì bản chất vẫn là bài đời sống, nick tech/SaaS nhảy vào comment sẽ rất lạc quẻ và lộ bot.
const LIFESTYLE_NOISE = [
  'kieu toc', 'cat toc', 'lam toc', 'haircut', 'uon toc', 'nhuom toc',
  'my pham', 'son moi', 'trang diem', 'skincare', 'lam dep',
  'thoi trang', 'quan ao', 'vay cuoi', 'outfit', 'phoi do',
  'showbiz', 'hoa hau', 'scandal', 'drama showbiz', 'dien vien', 'ca si',
  'mon an', 'nau an', 'quan an', 'an vat', 'am thuc', 'buffet',
  'tha thinh', 'tinh yeu', 'hen ho', 'tam su eva',
].map(stripDiacritics)
function isLifestyleNoise(normText) {
  for (const p of LIFESTYLE_NOISE) if (p && normText.includes(p)) return p
  return null
}
...
function screenPost(post, ctx, niche, aff = null) {
  ...
  // Loại bài đời sống / làm đẹp / thời trang / showbiz — tránh cmt nhảm vào bài cắt tóc, mỹ phẩm...
  const lifeNoise = isLifestyleNoise(norm)
  if (lifeNoise) return { eligible: false, reason: `lifestyle_noise:${lifeNoise}` }
  ...
  const khop = matchesAffinity(norm, aff)
  if (khop) return { eligible: true, tier: 'interest', matched: khop }
