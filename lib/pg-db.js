/**
 * pg-db.js — Direct PostgreSQL client for VPS SQL Database (103.142.24.60:5432/socialflow)
 * Uses native pg Pool instead of external cloud services.
 * Implements the fluent query builder API used throughout codebase.
 */
...
async function loadFKMap(pool) {
  ...
  try {
    ...
    console.log(`[PG-DB] FK map loaded: ${rows.length} relationships`)
  } catch (err) {
    console.warn(`[PG-DB] FK map load failed: ${err.message} — using heuristic`)
    _fkMap = new Map()
  }
  return _fkMap
}
