...
${langBlock}${adSection}
=== CÁCH CHẤM ĐIỂM ===
- 8-10: Người viết ĐANG HỎI / TÌM KIẾM giải pháp liên quan "${topic}" (hỏi mua VPS, tìm hosting, so sánh server) → ad_strategy="pitch"
- 6-7: Bài thảo luận / chia sẻ kinh nghiệm TRỰC TIẾP về "${topic}" (VPS lag, hosting sập) → ad_strategy="recommend"
- 5: Bài trong nhóm, liên quan chủ đề nhưng không có intent mua → ad_strategy="seed" hoặc "organic"
- 3-4: Bài quá chung chung, không có gì để bổ sung giá trị thực → BỎ QUA
- 1-2: Off-topic hoàn toàn, spam, quảng cáo của người khác → BỎ QUA

NGUYÊN TẮC CỐT LÕI:
- Score đánh giá KHẢ NĂNG COMMENT CÓ GIÁ TRỊ — không phải cơ hội quảng cáo.
- Bài hỏi cài đặt phần mềm, bug, lỗi app → vẫn score 5 nếu agent có thể reply hữu ích về mặt kỹ thuật, nhưng ad_strategy="organic".
- Mức độ quảng cáo (ad_strategy) hoàn toàn độc lập với score — score cao không có nghĩa là pitch.
- KHÔNG ép quảng cáo vào bài không liên quan chỉ để đạt KPI → nguy cơ ban nick.
...
Trả về JSON array:
[{"index": 1, "score": 8, "reason": "...", "action": "comment", "comment_angle": "...", "comment_language": "vi", "tier": 1, "ad_strategy": "pitch", "ad_opportunity": true, "matched_product_name": null, "ad_reason": "...", "lead_potential": false}]
...
CHỈ trả về JSON, không giải thích.`
  ...
  try {
    ...
    if (!results) {
      ...
      // Score threshold: lowered to 3 (was 5) so natural group discussion posts (score 3+) are selected.
      // Score 3+ = "general group discussion, can share a natural/organic response".
      const scoreThreshold = (typeof minScore === 'number') ? minScore : 3
      const filtered = results
        .filter(r => r.score >= scoreThreshold && r.index >= 1 && r.index <= posts.length && !adIdxSet.has(r.index))
      ...
      // FALLBACK: if nothing passes the strict threshold, pick the highest-scoring non-ad post (score >= 1)
      // to ensure natural group interaction and prevent perpetual 0-comment loops.
      const fallback = results
        .filter(r => r.index >= 1 && r.index <= posts.length && !adIdxSet.has(r.index))
        .map(r => ({ ...r, action: 'comment', ad_strategy: r.ad_strategy || 'organic' }))
        .sort((a, b) => b.score - a.score)
        .slice(0, Math.min(maxPicks || 2, 1)) // max 1 fallback to be conservative

      if (fallback.length > 0) {
        console.log(`[AI-BRAIN] 💡 Fallback active: Selected best available post #${fallback[0].index} (score=${fallback[0].score}) for natural organic interaction`)
        return fallback
      }
