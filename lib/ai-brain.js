...
- "ad_opportunity": Đặt là true CHỈ KHI tác giả bài viết ĐANG HỎI MUA / TÌM KIẾM / SO SÁNH VPS, Hosting, Server, Cloud — hoặc THAN PHIỀN VPS/Hosting của nhà cung cấp hiện tại bị lag/sập.
  BẮT BUỘC đặt là FALSE trong các trường hợp sau (kể cả khi bài ở trong group liên quan VPS):
  × Lỗi phần mềm / app / tool chạy trên localhost (ví dụ: "Internal Server Error" khi cài app, lỗi cấu hình 9router, OpenClaw bị crash)
  × Bài hỏi về cài đặt, cấu hình, debug một phần mềm cụ thể không liên quan đến việc MUA server
  × Lỗi API, Token, rate limit, OAuth, thiết kế UI, code HTML/JS
  × Thảo luận kỹ thuật không có ý định mua dịch vụ hosting
  × Từ khóa bẫy: "localhost", "internal server error", "cài đặt", "lỗi phần mềm", "npm", "port", "config" — những từ này KHÔNG = nhu cầu VPS
  Nếu không chắc chắn 100% → bắt buộc đặt là false.
- "matched_product_name": Tên chính xác của sản phẩm/gói cước phù hợp nhất từ danh sách trên (ví dụ: ${products.length > 0 ? `"${products[0].name}"` : '"null"'}). Nếu không khớp hoặc tier=3 hoặc ad_opportunity=false, hãy để null.
- "ad_reason": 1 câu giải thích tại sao chọn tier này, tại sao đặt ad_opportunity như vậy và lý do chọn sản phẩm/gói cước đó.
- "comment_angle": Gợi ý GÓC bình luận tự nhiên và phù hợp nhất.
  + Nếu ad_opportunity=true: Gợi ý góc đề xuất thương hiệu hoặc sản phẩm đã chọn dưới vai người dùng có kinh nghiệm thật (Ví dụ: "suggest gói [Tên Gói] của TinoHost để chạy tool ổn định, mượt mà 24/7").
  + Nếu ad_opportunity=false: Gợi ý góc bình luận nuôi nick tự nhiên, tập trung trả lời đúng kỹ thuật, đồng cảm, tuyệt đối KHÔNG nhắc tới thương hiệu/quảng cáo dù chỉ 1 chữ.
- "lead_potential": true nếu TÁC GIẢ bài viết thực sự là KHÁCH HÀNG TIỀM NĂNG — tức là họ đang có ý định mua/thuê dịch vụ (chỉ khi tier=1 hoặc tier=2 VÀ ad_opportunity=true).

QUAN TRỌNG VỀ PHÂN TÍCH NGỮ CẢNH:
- KHÔNG sử dụng keyword matching đơn thuần. Phải đọc kỹ toàn bộ nội dung bài và thread comments để hiểu chính xác vấn đề thực tế của tác giả.
- Bài viết trong group liên quan VPS KHÔNG có nghĩa là bài đó có ad_opportunity=true. Phải đọc NỘI DUNG CỤ THỂ.
- Ví dụ SAI: bài hỏi "9router bị Internal Server Error" → KHÔNG phải nhu cầu VPS → ad_opportunity=false, tier=3.
- Ví dụ ĐÚNG: bài hỏi "cần tìm VPS ổn định để chạy tool" → ad_opportunity=true, tier=1.
`
  }
...
${brandName ? `Thương hiệu chúng ta: ${brandName}${brandDescription ? ` — ${brandDescription}` : ''}. Chỉ đề cập thương hiệu khi ad_opportunity=true và nội dung bài THỰC SỰ liên quan đến nhu cầu mua/thuê dịch vụ.` : ''}
...
${langBlock}${adSection}
=== CÁCH CHẤM ĐIỂM ===
- 8-10: Người viết ĐANG HỎI / TÌM KIẾM giải pháp liên quan "${topic}" (hỏi mua VPS, tìm hosting, so sánh server) → comment trả lời cụ thể, CÓ THỂ đề xuất sản phẩm nếu ad_opportunity=true
- 6-7: Bài thảo luận / chia sẻ kinh nghiệm TRỰC TIẾP về "${topic}" → comment đồng tình hoặc bổ sung kỹ thuật
- 5: Bài trong nhóm, liên quan gián tiếp, hoặc hỏi về tool/phần mềm → comment nuôi nick (kỹ thuật, đồng cảm) — KHÔNG quảng cáo
...
- Bài hỏi cài đặt phần mềm, bug, lỗi app → vẫn score 5 nếu agent có thể reply hữu ích về mặt kỹ thuật.
- Quảng cáo (nhắc brand/sản phẩm) CHỈ xảy ra khi ad_opportunity=true — hoàn toàn độc lập với score.
...
Trả về JSON array:
[{"index": 1, "score": 8, "reason": "...", "action": "comment", "comment_angle": "...", "comment_language": "vi", "tier": 3, "ad_opportunity": false, "matched_product_name": null, "ad_reason": "...", "lead_potential": false}]
...
3. NEVER mention any brand names, product recommendations, or advertisements. This is a purely organic, brand-free nurture comment.
...
6. Write 1-2 sentences, short, casual abbreviations OK.
...
3. TUYỆT ĐỐI KHÔNG nhắc đến bất kỳ tên thương hiệu, sản phẩm quảng bá hay gợi ý mua hàng nào. Đây bắt buộc phải là bình luận chia sẻ thông tin thuần túy tự nhiên, thương hiệu sạch 100%.
4. Nếu bài hỏi kỹ thuật → trả lời kỹ thuật (config, command, số liệu)
5. Nếu bài chia sẻ kinh nghiệm → phản hồi ĐÚNG kinh nghiệm đó
6. Viết 1-2 câu, ngắn gọn, có thể dùng slang/viết tắt
...
VÍ DỤ ĐÚNG (trả lời đúng nội dung, không quảng cáo):
