...
CHỈ trả về JSON, không giải thích.`
  ...
  try {
    ...
    if (!results) {
      ...
      // FALLBACK: if nothing passes the threshold, return the highest-scoring posts anyway
      // (prevents perpetual 0-comment loops in groups with only medium-quality posts)
      ...
      if (fallback.length > 0) {
        console.log(`[AI-BRAIN] No posts scored >= ${scoreThreshold} — using fallback best post (score=${fallback[0].score})`)
        return fallback
      }

      console.log(`[AI-BRAIN] No posts scored >= ${scoreThreshold} — all scores: ${results.map(r => r.score).join(',')}`)
      return []
  } catch (err) {
