...
async function evaluatePosts({ posts, campaign, nick, group, topic, maxPicks, ownerId, adConfig, brandConfig, groupLanguage, minScore }) {
...
CHỈ trả về JSON, không giải thích.`
  ...
  try {
    ...
    if (!results) {
      ...
      // Accept any valid non-ad post scored >= 3 (or with action === 'comment')
      const scoreThreshold = (typeof minScore === 'number') ? minScore : 3
      const filtered = results
        .filter(r => r.index >= 1 && r.index <= posts.length && !adIdxSet.has(r.index))
        .filter(r => r.action === 'comment' || (r.score >= scoreThreshold && r.action !== 'skip'))
        .map(r => ({ ...r, action: 'comment', ad_strategy: r.ad_strategy || 'organic' }))
        .sort((a, b) => b.score - a.score)
        .slice(0, maxPicks || 2)

      if (filtered.length > 0) return filtered

      // FALLBACK: If AI returned skip for all, override best non-ad post to organic comment
      const fallback = results
        .filter(r => r.index >= 1 && r.index <= posts.length && !adIdxSet.has(r.index))
        .map(r => ({ ...r, action: 'comment', ad_strategy: 'organic', comment_angle: r.comment_angle || 'Bình luận tự nhiên như thành viên nhóm' }))
        .sort((a, b) => b.score - a.score)
        .slice(0, Math.min(maxPicks || 2, 1))

      if (fallback.length > 0) {
        console.log(`[AI-BRAIN] 💡 Organic fallback active: Selected post #${fallback[0].index} (score=${fallback[0].score}) for natural user comment`)
        return fallback
      }

      // SECONDARY FALLBACK: If AI omitted or failed indices, pick first eligible non-ad post
      const firstNonAdPost = posts.findIndex(p => !p.is_ad_post)
      if (firstNonAdPost !== -1) {
        const pIdx = firstNonAdPost + 1
        console.log(`[AI-BRAIN] 💡 Secondary fallback active: Selected first non-ad post #${pIdx} for organic interaction`)
        return [{
          index: pIdx,
          score: 3,
          reason: 'Fallback selection for natural group engagement',
          action: 'comment',
          comment_angle: 'Đồng cảm hoặc chia sẻ suy nghĩ ngắn gọn, tự nhiên về bài viết',
      ...
      console.log(`[AI-BRAIN] No eligible non-ad posts found — all scores: ${results.map(r => r.score).join(',')}`)
      return []
  } catch (err) {
...
async function qualityGateComment({ comment, postText, group, topic, nick, ownerId, threadComments, brandConfig }) {
  ...
  // Strict Brand Guard: Reject if comment contains brand keywords
  if (brandConfig?.brand_name) {
    ...
    // ── Lớp 1: Kiểm tra ngữ cảnh bài gốc ──────────────────────────────────
    // Nếu comment nhắc đến brand/sản phẩm, bài gốc PHẢI có tín hiệu mua/thuê dịch vụ
    ...
    if ((commentHasBrand || commentHasProduct) && !hasVpsIntent) {
      // Bài không có ngữ cảnh mua/tìm VPS → tuyệt đối không được nhắc brand
      ...
      console.warn(`[AI-BRAIN] Post snippet: "${postLower.substring(0, 100)}"`)
      return { approved: false, reason: `brand_on_offtopic_post: "${detected}" — bài gốc không có nhu cầu VPS/Hosting`, score: 1 }
    }

    // ── Lớp 2: Nurture comment không được chứa brand (ad_opportunity=false path) ──
    if (commentHasBrand) {
      console.warn(`[AI-BRAIN] 🛡️ Brand Guard Rejected Nurture Comment: "${comment}" (contained brand name: "${brandConfig.brand_name}")`)
      ...
      for (const prod of brandConfig.products) {
        if (prod?.name) {
          ...
          if (prodLower.length > 2 && commentLower.includes(prodLower)) {
            console.warn(`[AI-BRAIN] 🛡️ Brand Guard Rejected Nurture Comment: "${comment}" (contained product name: "${prod.name}")`)
...
4. Nếu bài hỏi kỹ thuật → trả lời kỹ thuật (config, command, số liệu)
5. Nếu bài chia sẻ kinh nghiệm → phản hồi ĐÚNG kinh nghiệm đó
6. Viết 1-3 câu, ngắn gọn, có thể dùng slang/viết tắt
