...
- "ad_opportunity": true nếu bài viết đang HỎI / TÌM KIẾM / THAN PHIỀN về vấn đề mà thương hiệu hoặc một trong các gói cước/sản phẩm trên giải quyết được.
- "matched_product_name": tên chính xác của sản phẩm/gói cước phù hợp nhất từ danh sách trên (ví dụ: ${products.length > 0 ? `"${products[0].name}"` : '"null"'}). Nếu không có sản phẩm cụ thể nào khớp xuất sắc hoặc không có danh sách sản phẩm, hãy để null.
- "ad_reason": 1 câu giải thích tại sao đây là cơ hội (hoặc tại sao không) và lý do chọn sản phẩm/gói cước đó.
- "comment_angle": nếu ad_opportunity=true → gợi ý GÓC comment tự nhiên nhất để gợi ý thương hiệu hoặc gói cước/sản phẩm đã chọn.
  Ví dụ: "suggest gói [Tên Gói] của Viettel như người đã dùng thật để giải quyết data lag", "chia sẻ đã dùng gói [Tên Gói] thấy mượt mà"
...
- KHÔNG dùng keyword matching đơn thuần — đánh giá theo NGỮ CẢNH thực sự
`
  }
...
Trả về JSON array:
[{"index": 1, "score": 8, "reason": "...", "action": "comment", "comment_angle": "...", "comment_language": "vi", "ad_opportunity": false, "matched_product_name": null, "ad_reason": "...", "lead_potential": false}]
