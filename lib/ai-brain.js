...
- "tier": Phân loại bài đăng thành 3 nhóm ngữ cảnh cụ thể:
  ...
  + 2 (Indirect Pain Point): Tác giả PHÀN NÀN về lỗi kỹ thuật, hiệu suất kém, hoặc sập/lag của nhà cung cấp hiện tại (ví dụ: "vps lag quá", "mạng chậm đơ", "sập web").
  + 3 (Informational & Casual Chat): Thảo luận thuần túy, chia sẻ kiến thức, hỏi kỹ thuật không liên quan thương mại (ví dụ: "học HTML", "chia sẻ code").
- "ad_opportunity": Đặt là true CHỈ KHI bài viết THỰC SỰ VÀ TRỰC TIẾP liên quan đến VPS, Hosting, Server, Cloud, hoặc treo tool trên VPS (Ví dụ: hỏi mua VPS, than phiền server sập/lag). Tuyệt đối đặt là false nếu bài viết chỉ nói về lỗi phần mềm thuần túy, API, tốn Token, thiết kế UI, code HTML/JS hoặc thảo luận chung chung không giải quyết được bằng VPS/Server. Nếu không chắc chắn hoặc chỉ liên quan gián tiếp, bắt buộc đặt là false.
...
- "comment_angle": Gợi ý GÓC bình luận tự nhiên và phù hợp nhất.
  ...
  + Nếu ad_opportunity=false: Gợi ý góc bình luận nuôi nick tự nhiên, tập trung trả lời đúng kỹ thuật, đồng cảm, tuyệt đối không nhắc tới thương hiệu/quảng cáo.
- "lead_potential": true nếu TÁC GIẢ bài viết thực sự là KHÁCH HÀNG TIỀM NĂNG (chỉ khi tier là 1 hoặc 2).

QUAN TRỌNG VỀ PHÂN TÍCH NGỮ CẢNH:
- KHÔNG sử dụng keyword matching đơn thuần. Phải đọc kỹ toàn bộ nội dung bài và lịch sử thảo luận (thread comments) để hiểu chính xác vấn đề thực tế của tác giả trước khi phân loại tier.
`
  }
...
async function qualityGateComment({ comment, postText, group, topic, nick, ownerId, threadComments, brandConfig }) {
  ...
  // Strict Brand Guard: Reject if pure nurture comment contains brand keywords
  if (brandConfig?.brand_name) {
    const brandNameLower = brandConfig.brand_name.toLowerCase().trim()
    const commentLower = comment.toLowerCase()
    
    // Check brand name
    if (brandNameLower.length > 2 && commentLower.includes(brandNameLower)) {
