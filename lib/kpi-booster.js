...
async function checkAndBoostKPI(supabase, options = {}) {
  ...
  try {
    ...
    // Dynamic interleave gap: stagger nicks by 3-6 minutes so they take turns (xen kẽ)
    // continuously throughout the day instead of waiting 45+ minutes idle in queue.
    // E.g., 3 nicks → Nick 1 (+30s), Nick 2 (+3.5m), Nick 3 (+7m), Nick 1 (+10.5m)...
    const interleaveGapSec = Math.max(180, Math.min(360, Math.floor(600 / activeNickCount)))

    console.log(`[KPI-BOOSTER] Evaluated ${shuffledRows.length} nick KPI row(s). Active nicks: ${activeNickCount}, Hours left today: ${hoursLeftToday}h, Interleave gap: ${interleaveGapSec}s.`)
