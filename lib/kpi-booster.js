...
async function checkAndBoostKPI(supabase, options = {}) {
  ...
  try {
    ...
    // Dynamic base gap: distribute jobs evenly over remaining hours among active nicks
    // Base gap per nick = max(2 min, remaining_hours * 3600 / (N * 6))
    const baseGapSec = Math.max(120, Math.floor((hoursLeftToday * 3600) / (activeNickCount * 6)))

    console.log(`[KPI-BOOSTER] Evaluated ${shuffledRows.length} nick KPI row(s). Active nicks: ${activeNickCount}, Hours left today: ${hoursLeftToday}h, Base gap: ${baseGapSec}s.`)
    ...
    for (const row of shuffledRows) {
      ...
      // FIX Bug 6: Use pre-loaded accMap instead of per-row DB query
      const acc = accMap[row.account_id]
      if (!acc || acc.status === 'checkpoint' || acc.status === 'expired') {
        console.log(`[KPI-BOOSTER] Account ${row.account_id.slice(0, 8)} is ${acc?.status || 'missing'}. Skipping boost.`)
        continue
      }

      // ── Enqueue Nurture Boost Job (for Comments & Likes) ──
      if (missingComments > 0 || missingLikes > 0) {
        ...
        if (!existingJobs || existingJobs.length === 0) {
          // Dynamic interval with random noise (15s–45s jitter) to eliminate HTTP 429 rate limit spikes
          const randomJitterMs = Math.floor(15000 + Math.random() * 30000)
          const staggerOffsetMs = (enqueuedCount * baseGapSec * 1000) + randomJitterMs
          const scheduledAtIso = new Date(Date.now() + staggerOffsetMs).toISOString()

          console.log(`[KPI-BOOSTER] 🎲 Nick ${row.account_id.slice(0, 8)} selected (randomized order). KPI: ${doneComments}/${tgtComments} comments. Auto-enqueuing job (+${Math.round(staggerOffsetMs / 1000)}s).`)
          const { error: insErr } = await supabase.from('jobs').insert({
            ...
            payload: {
              ...
              target_likes_remaining: missingLikes,
            },
            status: 'pending',
