...
const HARD_LIMITS = {
  ...
  feed_comment: { maxPerDay: 8,  maxPerSession: 2,  minGapSeconds: 2700 }, // 45 phút giữa 2 comment
 ...
 * @param {boolean} bypassSafety - if true, bypass age reduction
 * @returns {number} adjusted count
 */
function applyAgeFactor(count, nickAgeDays, bypassSafety = false) {
  if (bypassSafety) return count
  if (nickAgeDays < 14)  return Math.max(1, Math.floor(count * 0.5))
  if (nickAgeDays < 30)  return Math.max(1, Math.floor(count * 0.7))
  if (nickAgeDays < 90)  return Math.max(1, Math.round(count * 0.85))
  return count
}
...
function checkWarmup(actionType, nickAgeDays, bypassSafety = false) {
  if (bypassSafety || nickAgeDays >= 30) return { allowed: true, phase: 'mature' }
