...
const HARD_LIMITS = {
  ...
  // Lịch sử trần này: 60/ngày → sự cố checkpoint 14/08 → hạ 25/ngày, 4/phiên,
  // giãn 8 phút.
  //
  // NÂNG 50/ngày, 14/phiên theo yêu cầu user 27/08. LƯU Ý RỦI RO: 50 rất gần
  // mức 60 từng làm nick dính checkpoint. Khác biệt so với hồi đó, và là lý do
  // mức này còn chấp nhận được:
  //   - hồi 14/08 nick chưa xác minh tuổi (fb_created_at NULL) nên chạy full
  //     trần trên nick non; nay applyAgeFactor cắt mạnh nick <30 ngày.
  //   - comment nay qua cổng AI (post_eval chọn bài + quality_gate chấm) +
  //     guard tất định (echo/tên nick/bịa số/bịa tên miền), không còn đăng ồ ạt.
  //   - giãn cách giữa 2 comment do feed-seed rải, không dồn cục.
  // Nếu nick dính checkpoint lần nữa, ĐÂY là số đầu tiên cần hạ.
  feed_comment: { maxPerDay: 50, maxPerSession: 14, minGapSeconds: 480 },  // 8 phút giữa 2 comment
