...
async function callHermes(taskType, prompt, options = {}) {
  ...
  if (HERMES_ENABLED) {
    try {
      ...
      if (!silent) console.log(`[HERMES] task=${taskType}${accTag} → OK (${latencyMs}ms, ${text.length}ch)`)
      return { text, source: 'hermes', latencyMs, taskType, raw: res.data }
    } catch (err) {
...
async function generateReply({ message, context = '', language = 'vi', accountId }) {
  const prompt = `Reply to this ${language === 'en' ? 'comment' : 'bình luận'}:\n"${message}"\n${context ? `\nContext: ${context}` : ''}\nLanguage: ${language}\nKeep it natural and short.`
  return callHermes('reply_gen', prompt, { accountId, maxTokens: 100, temperature: 0.8 })
}

// Decide action for a post during nurture browsing
async function decideAction({ post, campaignTopic, accountId }) {
  const prompt = `Post: "${(post.text || '').substring(0, 400)}"\nAuthor: ${post.author || 'unknown'}\nReactions: ${post.reactions || 0}\nCampaign topic: ${campaignTopic}\n\nDecide action and return JSON.`
  return callHermesJson('action_decision', prompt, { accountId, maxTokens: 200, temperature: 0.3 })
}

// Score a group/post for topic relevance
async function scoreRelevance({ content, topic, accountId }) {
  const prompt = `Content: "${(content || '').substring(0, 500)}"\nTopic: ${topic}\n\nScore relevance and return JSON.`
  return callHermesJson('relevance_score', prompt, { accountId, maxTokens: 150, temperature: 0.1 })
}

// Score a lead (profile/member) for prospect quality
async function scoreLead({ profile, topic, accountId }) {
  const prompt = `Profile: ${JSON.stringify(profile).substring(0, 500)}\nTopic: ${topic}\n\nScore lead quality and return JSON.`
  return callHermesJson('lead_score', prompt, { accountId, maxTokens: 200, temperature: 0.2 })
}
